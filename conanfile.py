...
class LibtorchConan(ConanFile):
    ...
    def source(self):
        ...
        git_clone_command = (
            ...
            self.homepage, self._source_subfolder, self._source_subfolder, self.version
        )
        ...
        if not self.options.build_custom_protobuf:
            ...
            cleanup_command = "rm -rf {}/source_subfolder/third_party/protobuf".format(
                source_folder_path
            )
        ...
        with tools.environment_append(env_build):
            cmake = (
                CMake(
                    ...
                    parallel=self.options.enable_parallel_build,
                )
            ...
            if self.options.use_cuda:
                ...
                if self.options.use_cudnn:
                    ...
                    cmake.definitions["USE_STATIC_CUDNN"] = (
                        "ON" if self.options.use_static_cudnn else "OFF"
                    )
            ...
            cmake.configure(
                ...
                build_folder=os.path.join(
                    self.source_folder, self._source_subfolder, self._build_subfolder
                ),
            )
        return cmake
