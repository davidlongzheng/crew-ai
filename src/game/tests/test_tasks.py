...
def test_parse_formula():
    settings = Settings()
    for formula, desc in TASK_DEFS:
        AssignedTask(formula=formula, desc=desc, player=0, settings=settings)


def test_sum_condition():
    """Test '1T sum>28 #t=0' - must win a trick with sum > 28 and no trumps"""
    settings = Settings()
    task = AssignedTask(formula="1T sum>28 #t=0", player=0, settings=settings)
    state = MockState()
...
def test_consecutive_tricks():
    ...
    settings = Settings()
    task = AssignedTask(formula="consec(3)", player=0, settings=settings)
    state = MockState()
...
def test_specific_tricks():
    ...
    settings = Settings()
    task = AssignedTask(formula="T0 T1", player=0, settings=settings)
    state = MockState()
...
def test_specific_card():
    ...
    settings = Settings()
    task = AssignedTask(formula="9p", player=0, settings=settings)
    state = MockState()
...
def test_cumulative_tricks():
    ...
    settings = Settings()
    task = AssignedTask(formula="#T>#T(capt)", player=0, settings=settings)
    state = MockState()
...
def test_cumulative_cards():
    ...
    settings = Settings()
    task = AssignedTask(formula="#p>=5", player=0, settings=settings)
    state = MockState()
...
def test_with_condition():
    ...
    settings = Settings()
    task = AssignedTask(formula="1T with(t)", player=0, settings=settings)
    state = MockState()
...
def test_sweep_condition():
    ...
    settings = Settings(side_suit_length=3)  # Smaller suit length for testing
    task = AssignedTask(formula="#sweep>=1", player=0, settings=settings)
    state = MockState()
...
def test_no_lead_condition():
    ...
    settings = Settings()
    task = AssignedTask(formula="nolead(y,p)", player=0, settings=settings)
    state = MockState()
