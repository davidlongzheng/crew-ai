...
def test_rollout_deterministic():
    ...
    seed = 42
    result1 = do_rollout(settings=settings, seed=seed)
    result2 = do_rollout(settings=settings, seed=seed)
...
def test_rollout_structure():
    ...
    result = do_rollout(settings=settings, seed=42)
    ...
    assert len(result["rewards_pp"]) == settings.num_players
...
def test_rollout_rewards_range():
    ...
    assert all(reward == 0 for reward in result["rewards_pp"])


@pytest.mark.slow
def test_rollout_timing():
    """Test that do_rollout() executes within reasonable time."""
    import time
    ...
    for _ in range(num_iterations):
        ...
        do_rollout(settings=settings, seed=42)
